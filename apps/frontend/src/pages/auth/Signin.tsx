...
export function Signin() {
    ...
    // Auto‑focus the 2FA input when it appears
    useEffect(() => {
    ...
    const validateForm = () => {
        ...
        // If 2FA is required, only validate the code – credentials are already verified
        if (twoFactorRequired) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // Build payload – always include email & password, append 2FA code only when needed
        ...
        if (signInUser.fulfilled.match(result)) {
            // Successful authentication (no 2FA required or 2FA code verified)
    ...
    /**
     * Clears the 2FA/MFA state and re‑enables the email/password fields
     * without resetting their current values.
     */
    const handleBackToCredentials = () => {
        dispatch(resetTwoFactor());
        setTwoFactorCode("");
        setErrors((prev) => {
            const { twoFactorCode, ...rest } = prev;
            return rest;
        });
    };
        ...
        <div className="min-h-screen flex">
                ...
                <div className="relative z-10 flex flex-col justify-between p-16 w-full">
                    <Link to="/" className="flex items-center gap-3">
                        ...
                        <div className="flex items-center gap-0">
                            <span className="text-xl font-semibold tracking-tight text-nirex-text-primary">
                                {APP_NAME}
                            </span>
                            {APP_NAME_SUFFIX && (
                                <span className="font-mono text-[0.85em] text-nirex-text-primary">
                                    {APP_NAME_SUFFIX}
                                </span>
                            )}
                        </div>
                    ...
                    <div className="space-y-8">
                        ...
                        <blockquote className="text-2xl font-medium text-nirex-text-primary/90 leading-relaxed max-w-md">
                            "The most efficient way to manage and deploy your development
                            environments."
                        </blockquote>
                        <div className="flex items-center gap-4">
                            <div className="w-10 h-10 rounded-full bg-nirex-text-primary/10" />
                            <div>
                                <p className="text-nirex-text-primary font-medium">
                                    Sarah Chen
                                </p>
                    ...
                    <p className="text-nirex-text-muted text-sm">
            ...
            <div className="flex-1 flex flex-col justify-center items-center px-6 py-12 bg-nirex-base">
                ...
                >
                            ...
                            {twoFactorRequired ? (
                                <>
                                    ...
                                    <button
                                        ...
                                        className="text-nirex-text-primary font-medium hover:underline focus:outline-none"
                                    >
                                        Change credentials
                                    </button>
                                </>
                                ...
                                <>
                                    ...
                                    <Link
                                        to="/auth/signup"
                                        className="text-nirex-text-primary font-medium hover:underline"
                                    >
                                        Sign up
                                    </Link>
                                </>
                    ...
                    {/* Social Sign in – hide during 2FA */}
                    {!twoFactorRequired && (
                        <>
                            ...
                            <div className="relative mb-6">
                                ...
                                <div className="relative flex justify-center text-xs">
                                    <span className="bg-nirex-base px-2 text-nirex-text-muted">
                                        or
                                    </span>
                                </div>
                            </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        {/* Email – read‑only during 2FA */}
                        <div>
                            <label
                                htmlFor="email"
                                className="block text-sm font-medium text-nirex-text-primary mb-1.5"
                            >
                                Email
                            </label>
                            <div className="relative">
                                ...
                                <input
                                    ...
                                    className={`w-full h-10 pl-10 pr-4 rounded-lg border bg-nirex-base text-sm text-nirex-text-primary placeholder:text-nirex-text-muted focus:outline-none focus:ring-2 focus:ring-nirex-accent focus:border-transparent transition-all ${twoFactorRequired
                        ...
                        {/* Password – read‑only during 2FA */}
                        <div>
                            <div className="flex items-center justify-between mb-1.5">
                                <label
                                    htmlFor="password"
                                    className="block text-sm font-medium text-nirex-text-primary"
                                >
                                    Password
                                </label>
                                {!twoFactorRequired && (
                            ...
                            <div className="relative">
                                ...
                                <input
                                    ...
                                    readOnly={twoFactorRequired}
                                    placeholder="••••••••"
                                    className={`w-full h-10 pl-10 pr-10 rounded-lg border bg-nirex-base text-sm text-nirex-text-primary placeholder:text-nirex-text-muted focus:outline-none focus:ring-2 focus:ring-nirex-accent focus:border-transparent transition-all ${twoFactorRequired
                                        ? "opacity-60 cursor-not-allowed"
                                        : errors.password
                                            ? "border-destructive focus:ring-destructive/20"
                                            : "border-border"
                        ...
                        {/* 2FA Code – only visible when required */}
                        {twoFactorRequired && (
                            <div>
                                <label
                                ...
                                <div className="relative">
                                    <ShieldCheck className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-nirex-text-muted" />
                                    <input
                                        ...
                                        placeholder="123456"
                                        className={`w-full h-10 pl-10 pr-4 rounded-lg border bg-nirex-base text-sm text-nirex-text-primary placeholder:text-nirex-text-muted focus:outline-none focus:ring-2 focus:ring-nirex-accent focus:border-transparent transition-all ${errors.twoFactorCode
                                            ? "border-destructive focus:ring-destructive/20"
                                            : "border-border"
                                            }`}
                                    />
                        ...
                        {/* Remember me – only on initial login */}
                        {!twoFactorRequired && (
                            <div className="flex items-center">
                                ...
                                <label
                                    htmlFor="remember"
                                    className="ml-2 text-sm text-nirex-text-secondary"
                                >
                    ...
                    <p className="mt-6 text-center text-xs text-nirex-text-muted">
                        ...
                        <Link
                            ...
                            className="text-nirex-text-primary hover:underline"
                        >
                            Terms
                        </Link>{" "}
                        and{" "}
