...
import nirex from "./theme/nirex.json" with { type: "json" }
  ...
  init: (props: { mode: "dark" | "light" }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
    const active = config.theme ?? kv.get("theme", "nirex")
    draft.active = typeof active === "string" ? active : "nirex"
        draft.ready = false
      }),
    )
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.nirex, store.mode)
    })
