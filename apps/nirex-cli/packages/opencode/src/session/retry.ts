...
export const GO_UPSELL_URL = "https://nirex.ai/go"
...
export function retryable(error: Err, provider: string) {
  ...
  if (SessionLegacy.APIError.isInstance(error)) {
    ...
    if (error.data.responseBody?.includes("FreeUsageLimitError")) {
      return {
        ...
        action: {
          ...
          message: "Subscribe to Nirex Go for reliable access to the best open-source models, starting at $5/month.",
          label: "subscribe",
          link: GO_UPSELL_URL,
        },
      }
    ...
    if (error.data.responseBody?.includes("GoUsageLimitError")) {
      ...
      const link = `https://nirex.ai/workspace/${workspace}/go`
      return {
