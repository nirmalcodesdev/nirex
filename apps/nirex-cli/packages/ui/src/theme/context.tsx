...
import nirexThemeJson from "./themes/nirex.json"
...
const STORAGE_KEYS = {
  THEME_ID: "nirex-theme-id",
  COLOR_SCHEME: "nirex-color-scheme",
  THEME_CSS_LIGHT: "nirex-theme-css-light",
  THEME_CSS_DARK: "nirex-theme-css-dark",
} as const
...
const fallbackTheme = nirexThemeJson as DesktopTheme
...
export const { use: useTheme, provider: ThemeProvider } = createSimpleContext({
  ...
  init: (props: { defaultTheme?: string; onThemeApplied?: (theme: DesktopTheme, mode: "light" | "dark") => void }) => {
    const themeId = normalize(read(STORAGE_KEYS.THEME_ID) ?? props.defaultTheme) ?? "nirex"
    ...
    const onStorage = (e: StorageEvent) => {
      if (e.key === STORAGE_KEYS.THEME_ID && e.newValue) {
        ...
        if (!next) return
        if (next !== "nirex" && !knownThemes().has(next) && !store.themes[next]) return
    ...
    onMount(() => {
      ...
      const savedTheme = normalize(rawTheme ?? props.defaultTheme) ?? "nirex"
    ...
    const setTheme = (id: string) => {
      ...
      if (next !== "nirex" && !knownThemes().has(next) && !store.themes[next]) {
        console.warn(`Theme "${id}" not found`)
        return
      }
      setStore("themeId", next)
